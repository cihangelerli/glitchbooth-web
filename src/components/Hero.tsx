...
export default function Hero({
  ...
  return (
    <motion.div
      ...
      className="relative w-full bg-black/95 border-2 border-[#00ff41] p-6 md:p-12 glow-border-matrix text-left cursor-default select-none"
      style={{ borderRadius: "0px" }}
      onClick={(e) => e.stopPropagation()} // Keeps inner clicks safe from triggering the backdrop dismiss
    >
      {/* Brutalist Close Button Trigger */}
      <button
        onClick={onClose}
        className="absolute top-3 right-3 text-[#84967e] hover:text-[#00ff41] transition-colors p-1 font-mono text-xs flex items-center space-x-1 border border-transparent hover:border-[#00ff41]/30 bg-black cursor-pointer"
        title="Dismiss Terminal Interface"
      >
        <X size={14} />
        <span className="hidden sm:inline text-[10px]">[ESC_DISMISS]</span>
      </button>

      {/* Terminal Header Info */}
      <div className="flex flex-wrap items-center justify-between text-[9px] md:text-xs font-mono text-[#84967e] mb-6 md:mb-8 border-b border-matrix/10 pb-4 pr-16 sm:pr-24">
      ...
      {/* Glitch Title */}
      <h1 className="font-display text-4xl sm:text-5xl md:text-6xl font-extrabold text-[#00ff41] tracking-wider mb-6 relative leading-none text-center">
        <span className="relative inline-block glow-text-matrix select-all">
          {glitchTitle}
          <span
      ...
      <p className="font-mono text-sm sm:text-base md:text-lg text-[#e2e2e2] leading-relaxed max-w-2xl mx-auto mb-8 md:mb-10 text-center pr-2">
        A raw, high-contrast digital brutalism experience. Capturing moments in
        the tension between perfect execution and hardware failure.
      </p>
