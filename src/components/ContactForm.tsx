...
export default function ContactForm() {
  ...
  const handleTransmit = (e: React.FormEvent) => {
    ...
    setLogLines([
      ...
      'RESOLVING IMAGEKIT GATEWAY ROUTE...',
    ]);

    // Simulate brutalist terminal uploading steps
    setTimeout(() => {
      setLogLines(prev => [...prev, 'PACKET_TRANSMISSION: SENT_OK']);
    }, 600);

    setTimeout(() => {
      setLogLines(prev => [...prev, 'SYNCHRONIZING TERMINAL OVERFLOW FEED...']);
    }, 1200);
      ...
      >
        ...
        <div className="text-center mb-8 md:mb-12">
          ...
          <p className="font-mono text-xs sm:text-sm text-[#b9ccb2] max-w-md mx-auto leading-relaxed">
            Connect your terminal to our live feed and participate in the global digital decay.
          </p>
        </div>
        ...
        <form onSubmit={handleTransmit} className="space-y-6 text-left">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4 border-t border-matrix/10">
            ...
            {/* Status logs block dynamically rendering during transmitting state */}
                ...
                {status === 'success' && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-[9px] text-[#b9ccb2]">Your transmission hash was catalogued in our local CRT registry buffer.</div>
                    </div>
                  </motion.div>
                )}
                ...
                {status === 'error' && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-[9px] text-magenta">Please check your parameters and retry transmission.</div>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            {/* Submit button on right bottom, absolute brutalist block style */}
            <button
              type="submit"
              disabled={status === 'transmitting'}
